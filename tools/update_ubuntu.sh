...
for ubuntuver in bionic focal; do
    case "${ubuntuver}" in
    "bionic")
        regex="(linux-image-unsigned-(4.15.0|5.4.0)-.*-(generic|azure|gcp)-dbgsym|linux-image-(4.15.0|5.4.0)-.*-aws-dbgsym)"
        ubuntu_number=18.04
        ;;
    "focal")
        regex="(linux-image-unsigned-(5.4.0|5.8.0|5.11.0)-.*-(generic|azure|gcp)-dbgsym|linux-image-(5.4.0|5.8.0|5.11.0)-.*-aws-dbgsym)"
        ubuntu_number=20.04
        ;;
    *)
    continue
    ...
    grep "Package:" ./packages | sed 's:Package\: ::g' | sed 's:linux-image-::g' | sed 's:-dbgsym.*::g' | sed 's:unsigned-::g' | sort > packages_version
    ...
    rm -f ${ubuntuver} ${ubuntuver}-updates gs_names packages_version
